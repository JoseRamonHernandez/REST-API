...
export const coursesCompleted = async (req, res) => {
  const { id } = req.params; // id del colaborador existente
  const { idCategorie, id_course, calf } = req.body; // nuevos registros 
  ...
  try {
    ...
    registroExistente.courses_completed.push({idCategorie: idCategorie, id_course: id_course, calf: calf });
...
export const insertDocument = async (req, res) => {
  const { id } = req.params; // id del colaborador existente
  const { text_document, url_document } = req.body; // nuevos registros 
  ...
  try {
    ...
    registroExistente.documents.push({ text_document: text_document, url_document: url_document });

    await registroExistente.save();
    res.json({ mensaje: 'Documento registrado para el colaborador' });
...
/*Get all DOCUMENTS into collaborator*/
export const showDocuments = async (req, res) => {
  ...
  try {
    ...
    return res.json(collaborator.documents)
  } catch (error) {
    return res.status(500).json({ message: error.message })
  }
...
//Eliminamos el DOCUMENTS del colaborador
export const deleteDocument = async (req, res) => {

  const { idCollaborator, idDocument } = req.params;

  try {
    const collaborator = await Collaborator.findById(idCollaborator);

    if (!collaborator) {
      return res.status(404).json({ message: 'Colaborador no encontrado' });
    }

    const documentIndex = collaborator.documents.findIndex(doc => doc._id.toString() === idDocument);

    if (documentIndex === -1) {
      return res.status(404).json({ message: 'Documento no encontrado' });
    }

    collaborator.documents.splice(documentIndex, 1);
    await collaborator.save();

    return res.status(200).json({ message: 'Documento eliminado exitosamente' });
  } catch (error) {
    return res.status(500).json({ message: 'Error al eliminar el documento', error });
  }
...
export const insertMaterial = async (req, res) => {
  try {
    const categoria = await Categories.findById(req.params.id);
    const { name, documentType, url } = req.body; // nuevos campos a agregar
    // Verificamos que la categoría exista
    if (!categoria) {
      return res.status(404).json({ message: 'La categoría no existe' });
    }
    ...
    curso.material.push({ name: name, documentType: documentType, url: url });
...
/*Actualizar porcent de una Operation por id y por numero de empleado */
...
/* Traer todos los Aplicators*/
export const getAplicators = async (req, res) => {

  try {
    const aplicators = await Solicitud.find()
    res.json(aplicators)
  } catch (error) {
    return res.status(500).json({ message: error.message })
  }
...
/* Crer un Aplicator*/
export const createAplicator = async (req, res) => {

  try {
    const { id_collaborator, id_categorie, status } = req.body

    const aplicator = new Solicitud({
      id_collaborator,
      id_categorie,
      status
    })
    await aplicator.save()
    res.send({ "code": 201, "message": "Aplicator inserted successfully" })
  } catch (error) {
    return res.status(500).json({ message: error.message })
  }
}

/*Actualizar un Aplicator*/
export const updateAplicator = async (req, res) => {

  try {
    const { id } = req.params

    const aplicatorUpdated = await Solicitud.findByIdAndUpdate(id, req.body, {
      new: true
    })
    return res.json(aplicatorUpdated)
  } catch (error) {
    return res.status(500).json({ message: error.message })
  }
}
...
/*Eliminar un Aplicator*/
export const deleteAplicator = async (req, res) => {

  try {
    const aplicator = await Solicitud.findByIdAndDelete(req.params.id)

    if (!aplicator) {
      return res.status(404).json({
        "message": "Aplicator doesn´t exists"
      })
    }

    return res.send({ "code": 202, "message": "Aplicator was deleted" })
  } catch (error) {
    return res.status(500).json({ message: error.message })
  }
}
...
/*Encontrar un Aplicator por id*/
export const findAplicatorID = async (req, res) => {

  try {
    const aplicator = await Solicitud.findById(req.params.id)
